...
Перед выполнением нетривиальной задачи оцени, подходит ли какая-то возможность oh-my-claudecode (OMC), и **предложи пользователю** подходящий режим одной короткой фразой до начала работы. Не запускай автоматически — сначала получи согласие.
...
- `/trace`, `/debug` — эвиденс-цепочная диагностика
